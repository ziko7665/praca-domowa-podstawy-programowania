...
void ustawienia_XO(bool &XO_ods, bool &XO_graf, int &XO_czas)
{
    ...
    while (true)
    {
        ...
        if (XO_ods == true)
            ...
            cout << "OFF" << endl;
        if (XO_czas > 0)
            cout << "- Czas na Gracza: " << XO_czas << "s" << endl;
        else
            cout << "- Czas na Gracza: OFF" << endl;
        ...
        if (tryb.size() == 1 && (tryb[0] == '1' || tryb[0] == '2' || tryb[0] == '3' || tryb[0] == 'X' || tryb[0] == 'x'))
        {
            ...
            case '3':
            {
                ...
                while (true)
                {
                    cout << "Podajac 0, usuwasz limit czasu" << endl
                         << endl;
...
void XO_plansza(char XO[3][3], char graf_plansza_XO[3][3], int czasO, int czasX, int XO_czas)
{
    ...
    if (czasO == -1)
        cout << "                   " << endl; // brak wypisania czasu kiedy czas jest -1(komputer gra tym znakiem)
    else
    {
        cout << "Czas gracza O: ";
    ...
    if (czasO == -1)
        SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), COORD{(SHORT)x, (SHORT)0}); // ustawienie kursora w miejsce czasu O gdy komputer gra jako O
    if (czasX == -1)
        cout << "                   " << endl;
    else
    {
